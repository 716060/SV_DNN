...
filenames, spk_names  = list_file_parse(list_file)
# [filenames, feature_len], spk_names  = list_file_parse(list_file)
# feature_len = list(map(tf.int32, feature_len))
...
def feature_queue(feature):
    # convert the feature to a tensor
    feature_tensor = tf.convert_to_tensor(feature, dtype=tf.float32)
    # randomize the tensor
    # tf.random_shuffle(feature_len_tensor)
    # create the queue
    featture_q = tf.FIFOQueue(shapes=[200, 39], capacity=128*1, dtypes=tf.float32)
    # create our enqueue_op for this q
    feature_q_enqueue_op = featture_q.enqueue_many([feature_tensor])
    # create a QueueRunner and add to queue runner list
    # we only need one thread for this simple queue
    tf.train.add_queue_runner(tf.train.QueueRunner(featture_q, [feature_q_enqueue_op] * 1))
    return featture_q
...
record_defaults = [[1.0] for cl in range(200*128*1)]
...
#                                    dataset_path='/dataset/ofyfa',
#                                    input_transform=None,
#                                    batch_size=10)
# truth_batch = loader.dequeue()
...
# feature_len_queue = feat_len_queue(one_frame)
# feat_one_batch = feature_len_queue.dequeue()

features = gen_batch.read_multiline([one_frame],
            batch_size=39)

features = tf.transpose(features)
features = tf.reshape(features,[1*128, 200, 39])
features_queue = feature_queue(features)
features_segment = features_queue.dequeue()
features_batch, label_batch = tf.train.batch(
            [features_segment, spk_name],
            batch_size=128,
            num_threads=2,
            capacity=1 + 3 * 128)
